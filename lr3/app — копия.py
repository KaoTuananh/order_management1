...
class Application:
    ...
    def __call__(self, environ, start_response):
        ...
        if path == '/' or path == '/index':
            ...
            response = self.main_controller.handle_request(environ)
        elif path == '/delete':  # НОВЫЙ МАРШРУТ ДОБАВЛЕН
            response = self.main_controller.handle_request(environ)
        elif path == '/add':
...
def run_server(host='localhost', port=8000):
    ...
    print("  POST /delete              - Удаление клиента (AJAX)")  # НОВЫЙ МАРШРУТ
    ...
    print("  ✓ Удаление клиентов через AJAX")
    print("  ✓ Подтверждение перед удалением")
    print("=" * 60)
